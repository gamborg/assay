...
import { printSuite, printResult, printTrace, createStreamReporter } from '../report/console.js';
...
async function commandRun(positional, flags) {
  ...
  const suite = await runSuite({
    ...
    onResult: streaming ? createStreamReporter() : undefined,
  });

  if (flags.json) {
    console.log(JSON.stringify(suite, null, 2));
  } else {
    printSuite(suite, console.log, { results: false });
    if (flags.trace) {
...
async function commandBaseline(flags) {
  ...
  const suite = await runSuite({ scenarios, targets, onResult: createStreamReporter() });
  const file = await saveBaseline(suite);
...
/**
 * A flag written as `--save false` arrives as the *string* "false", which is
 * truthy. Treating that as "on" is the kind of quiet surprise that makes people
 * distrust a CLI, so negation is explicit and covers both spellings.
 */
function isOff(value) {
  return value === false || value === 'false' || value === 'no' || value === 'off';
}
...
Options
  ...
  --save false            Do not write this run to .assay/runs/
